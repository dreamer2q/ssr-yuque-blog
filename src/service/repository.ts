...
export class RepositoryService {
  ...
  async getDocs({ offset = 0, limit = 20 }: { offset: number; limit: number }) {
  ...
  async getDocDetail(slug: string) {
    const getV2 = this.http2.get(`/repos/dreamer2q/blog/docs/${slug}`);
    const getV1 = this.http.get(`/docs/${slug}`, {
      params: {
        book_id: 2030975,
        // include_contributors: true,
        include_hits: true,
    ...
    const [doc2, doc] = await Promise.all([getV2, getV1]);
